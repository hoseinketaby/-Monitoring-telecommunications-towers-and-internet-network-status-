...
import { LayoutDashboard, Map, RefreshCw, RadioTower } from 'lucide-react'
...
export default function App() {
  const [page, setPage] = useState<'map' | 'dashboard'>('dashboard')
    ...
    <main dir="rtl" className="min-h-screen bg-canvas text-slate-100">
      <header className="sticky top-0 z-[1100] border-b border-line bg-canvas/90 backdrop-blur">
        <div className="mx-auto flex max-w-[1600px] flex-wrap items-center gap-3 px-4 py-3">
          ...
          <nav className="flex rounded-lg border border-line p-1 text-sm">
            <button onClick={() => setPage('dashboard')} className={`inline-flex items-center gap-2 rounded px-3 py-1.5 ${page === 'dashboard' ? 'bg-slate-700' : 'text-slate-400'}`}><LayoutDashboard className="h-4 w-4" /> داشبورد</button>
            <button onClick={() => setPage('map')} className={`inline-flex items-center gap-2 rounded px-3 py-1.5 ${page === 'map' ? 'bg-slate-700' : 'text-slate-400'}`}><Map className="h-4 w-4" /> نقشه و هشدارها</button>
          </nav>
      ...
      <div className="mx-auto max-w-[1600px] p-4">{page === 'dashboard' ? <Dashboard /> : <MapView />}</div>
    </main>
  )
}
