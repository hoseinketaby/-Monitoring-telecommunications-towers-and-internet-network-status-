...
export function generateMockTowers(count = 20): TowerState[] {
  return Array.from({ length: Math.max(count, 20) }, (_, index) => {
    ...
    return {
      ...
      name: `${region}-${index + 1}`,
      lat: lat + (Math.random() - 0.5) * 0.28,
      lng: lng + (Math.random() - 0.5) * 0.28,
      region,
      status: 'online',
      signalStrength: random(72, 99),
      packetLoss: Number((Math.random() * 1.6).toFixed(1)),
      cpuTemp: random(28, 40),
      connectedUsers: random(80, 700),
      bandwidthUsageMbps: random(30, 360),
      weather: defaultWeather,
      isGridPowerActive: true,
      batteryLevel,
      batteryHealth: isOld ? random(70, 85) : random(86, 100),
      estimatedRuntimeMinutes: null,
      outageStartedAt: null,
      installedAt: `${isOld ? 2020 : 2024}-01-01T00:00:00.000Z`,
      dataSource: 'mock',
